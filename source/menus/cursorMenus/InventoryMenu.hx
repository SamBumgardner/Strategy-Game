...
class InventoryMenu extends CursorMenuTemplate implements VarSizedBox
{
	 ...
	 * 	between item boxes.
	 */
	private var textSize(default, never):Int = 32;
	...
	private function initInventorySlots(X:Float, Y:Float):Void
	{
		...
		var menuOptionWidth:Float = boxWidth - cornerSize;
		...
		for (i in 0...maxInventorySlots)
		{
			...
			newMenuOption = new MenuOption(slotX, slotY, i, menuOptionWidth, "", textSize);
